...
class Object:
    ...
    def __str__(self):
        return (
            f"{self.origin[0]:.2f} {self.origin[1]:.2f} {np.rad2deg(self.angle):.2f} "
...
class Localizer:
    ...
    def _update_input_image_parameters(self, image_shape):
        ...
        input_size = tuple(
            np.ceil(np.array(image_shape[:2]) * self._input_scale).astype(int)
        )
        ...
        self._image_t_output = np.dot(
            np.linalg.inv(self._input_t_image), input_t_output
        )
    ...
    def _create_model(self):
        ...
        output_pos = model.output[
            :, :, :, :, TrainingModelChannels.Y : TrainingModelChannels.X + 1
        ]
        output_angle = model.output[
            :, :, :, :, TrainingModelChannels.SA : TrainingModelChannels.CA + 1
        ]

        scale = np.full(2, 1.0 / self._sigma)
        ...
        confidence_map = tf.nn.conv2d(
            output_window_pos,
            np.expand_dims(confidence_kernel, 3),
            strides=1,
            padding="SAME",
            name="confidence_map",
        )
        ...
        confidence_loc_max = (
            tf.nn.max_pool(confidence_map, 3, 1, "SAME", "NCDHW") - confidence_map
        ) == 0
        confidence_thr = tf.math.logical_and(
            confidence_loc_max, confidence_map >= self._cfg["confidence_thr"]
        )
        ...
        def make_average_function(output, ksize, name):
            ...
            average_func = tf.nn.conv2d(
                output, kernel, strides=1, padding="SAME", name=name
            )
            return average_func

        average_pos = make_average_function(
            output_pos, self._pos_ksize, name="average_pos"
        )
        average_angle = make_average_function(
            output_angle, self._angle_ksize, name="average_angle"
        )

        confidence = tf.squeeze(confidence, -1)
        local_max_idx = tf.where(confidence)
        category = tf.cast(local_max_idx[:, 1], tf.float32)
        yx = tf.gather_nd(average_pos, local_max_idx) + tf.cast(
            local_max_idx[:, 2:], tf.float32
        )
    ...
    def predict(self, image):
        ...
        if self._diag:
            batch.output_on_image = np.zeros(
                (batch_size, self._category_count, 2) + image.shape
            )
            ...
            for bi in range(batch_size):
                ...
                if self._diag:
                    batch.outputs = np.expand_dims(
                        result[PredictionModelOutputs.MODEL][bi], 0
                    )
                    batch.output_window_pos = np.expand_dims(
                        result[PredictionModelOutputs.OUTPUT_WINDOW_POS][bi], 0
                    )
                    ...
                    for cat in range(self._category_count):
                        batch.output_on_image[bi][cat][0] = blend(
                            image, batch.outputs[0, cat, :, :, 0], 1
                        )
                        batch.output_on_image[bi][cat][1] = blend(
                            image, batch.outputs[0, cat, :, :, 1], 1
                        )

                    utils.save_batch_as_images(
                        batch,
                        self._diag_dir,
                        prefix="{:02}".format(bi),
                        fmt="{0}{2}.png",
        ...
        for i in range(len(objects)):
            predictions.append(
                Object(
                    pos[i][0], pos[i][1], objects[i][2], objects[i][3], objects[i][4]
                )
            )
