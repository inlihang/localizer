...
class Batch:
    ...
    def __init__(self, size, input_shape, output_shape):
        ...
        self.weight = np.empty(
            shape=(size,) + output_shape[:3] + (1,), dtype=np.float32
        )
...
class BatchGenerator:
    ...
    def generate_batch(self, batch_size, filter_id, **kwargs):
        ...
        assert (
            flt.data_element_weights is not None
        ), "Have you forgotten to initialize the probability array?"
        data_element_indices = self._rng.choice(
            flt.data_element_indices, size=batch_size, p=flt.data_element_weights
        )
...
class MultiProcessBatchGenerator:
    ...
    def _add_new_jobs(self):
        ...
        for b in range(new_jobs_count):
            job = self._pool.apply_async(
                _worker_process_generate_batch, (self._generate_batch_args,)
            )
...
class CategoryStatistics:
    ...
    def get_text(self):
        ...
        text += "{0:7}|{1:6.2f}|{2:7}|{3:6.2f}".format(
            self.missing, self.missing * total_pc, self.ufo, self.get_ufo_percentage()
        )
    ...
    def get_misclassified_text(self):
        ...
        for c in categories:
            text += (
                f"{self.category:3}->{c:3} {self.misclassified.get(c, 0):5} "
    ...
    def add(self, other):
        ...
        self.max_err_orientation = max(
            self.max_err_orientation, other.max_err_orientation
        )
...
class Trainer:
    ...
    def _read_dataset(self):
        ...
        if training_fraction == int(training_fraction):
            hashes = [
                (
                    random.str_to_random(
                        os.path.basename(self._dataset.data_elements[i].rel_path)
                    ),
                    i,
                )
        ...
        for i, data_element in enumerate(self._dataset.data_elements):
            if type(training_fraction) == list:
                ...
                accepted = (
                    random.str_to_random(os.path.basename(data_element.rel_path))
                    < training_fraction
                )
            if accepted:
        ...
        for flt_name, flt in self._filters.items():
            ...
            for i, data_element_idx in enumerate(flt.data_element_indices):
                for obj in self._dataset.data_elements[data_element_idx].objects:
                    flt.data_element_weights[i] += (
                        1 / total_obj_count_by_cat[obj.category] / self._category_count
                    )
    ...
    def _make_model(self):
        ...
        self._image_input = keras.Input(
            shape=[None, None, self._cfg["input_shape"][2]],
            dtype="float32",
            name="image",
        )
        ...
        logger.info(
            f"Output shape: {self._model.output.shape}, parameters: {self._model.count_params()}"
        )
        logger.info(
            f"Feature model parameters: {self._features_model.count_params()} "
            f"({self._features_model.count_params() / self._model.count_params() * 100:.3f}%)"
        )

        self._target_window_input = keras.Input(
            shape=self._model.output.shape[1:], dtype="float32", name="target_window"
        )
        self._weight_input = keras.Input(
            shape=self._model.output.shape[1:-1] + (1,), dtype="float32", name="weight"
        )
    ...
    def _create_model(self):
        ...
        if "transfer_learning_base" in self._cfg:
            self._features_model = keras.models.load_model(
                self._cfg["transfer_learning_base"]
            )
            ...
            f = self._image_input - tf.constant(
                self._dataset.image_mean, name="image_mean"
            )
        ...
        for cat in range(self._category_count):
            ...
            category_model = keras.layers.Conv2D(32, (1, 1), **conv_params)(
                category_model
            )
            category_model = keras.layers.Conv2D(
                predict.TrainingModelChannels.COUNT, (11, 11), **conv_params
            )(category_model)
        ...
        self._model = keras.Model(
            inputs=self._image_input, outputs=category_models, name="model"
        )
    ...
    def _create_loss(self):
        ...
        loss_weight = tf.constant(
            [loss_weight_pos, loss_weight_pos, loss_weight_angle, loss_weight_angle],
            dtype="float32",
        )
        ...
        self._loss = keras.Model(
            inputs=(self._image_input, self._target_window_input, self._weight_input),
            outputs=(loss, self._model.output),
            name="loss",
        )
        ...
        self._diag_window_image_input = keras.Input(
            self._model.output.shape[1:], dtype="float32"
        )
        diag_window_func = make_window(self._diag_window_image_input)
        self._diag_window_func = keras.Model(
            inputs=self._diag_window_image_input, outputs=diag_window_func
        )
        # TODO(ia): make working with TF 2.4
    ...
    def _train(self):
        ...
        for p in train_phase_params:
            total_training_examples_count += self._cfg[
                p["name"] + "_training_examples_count"
            ]
        ...
        self._batchgen = MultiProcessBatchGenerator(
            BatchGenerator(
                self._cfg, self._dataset.data_elements, self._filters.values()
            ),
            self._cfg["process_count"],
        )

        self._optimizer = keras.optimizers.Adam(
            learning_rate=self._cfg.get("learning_rate", 0.0005)
        )
    ...
    def _train_step(self, inputs, target_windows, weights):
        with tf.GradientTape() as tape:
            loss_value, outputs = self._loss(
                (inputs, target_windows, weights), training=True
            )
    ...
    def _train_phase(self, train_phase_params):
        training_examples_to_make = self._cfg[
            train_phase_params["name"] + "_training_examples_count"
        ]
        ...
        logger.info(
            f"Validate on {self._filters[validate_filter_name].size} data elements."
        )
        ...
        train_progress_logger.info(
            f"Examples\t {CategoryStatistics.HEADER}".replace("|", "\t")
        )
        ...
        while not is_phase_finished():
            ...
            self._batchgen.start_generation(
                self._cfg["training_examples_per_batch"], train_filter_name
            )
            training_examples_in_epoch_done = 0
            # Loop over batches in epoch
            while (
                not is_phase_finished()
                and training_examples_in_epoch_done
                < self._cfg["training_examples_per_epoch"]
            ):
                ...
                if training_examples_in_epoch_done == len(batch.input):
                    ...
                    te_dir = os.path.join(
                        self._output_dir, TRAINING_EXAMPLES_DIAG_DIR_NAME
                    )
            ...
            logger.info(
                f"Training examples: {self._training_examples_done}, loss: {self._epoch_loss.result():.6f}, "
            ...
            loss_line.set_xdata(
                np.append(loss_line.get_xdata(), self._training_examples_done)
            )
            loss_line.set_ydata(
                np.append(loss_line.get_ydata(), self._epoch_loss.result())
            )
            loss_line.axes.set_ylim([0, loss_line.get_ydata().max() * 1.1])
            found_line.set_xdata(
                np.append(found_line.get_xdata(), self._training_examples_done)
            )
            found_line.set_ydata(
                np.append(
                    found_line.get_ydata(),
                    stats["ALL"].found / stats["ALL"].total * 100,
                )
            )
            ufo_line.set_xdata(
                np.append(ufo_line.get_xdata(), self._training_examples_done)
            )
            ufo_line.set_ydata(
                np.append(ufo_line.get_ydata(), stats["ALL"].get_ufo_percentage())
            )
            plt.draw()
            plt.pause(0.001)
    ...
    def _create_plot(self, train_phase_params):
        ...
        plt.xlim(
            [
                0,
                self._cfg[train_phase_params["name"] + "_training_examples_count"]
                * 1.1,
            ]
        )
        plt.ylim([0, 105])
    ...
    def _validate_model(self, train_phase_params, extended_validation):
        ...
        for i in validate_filter.data_element_indices:
            ...
            image_file = data_element.rel_path.replace(os.path.sep, "_").replace(
                "/", "_"
            )
            image_count += 1
            ...
            self._log(
                f"Image file {data_element.rel_path}, ground truth objects: {len(gt_objects)}, "
                f"predicted objects {len(pr_objects)}",
                "d",
            )
            ...
            if extended_validation:
                ...
                cv2.imwrite(
                    os.path.join(result_dir, image_file) + ".png",
        ...
        for gt_object in gt_objects:
            ...
            if gt_object.match is None:
                ...
                if pr_object.category == gt_object.category:
                    ...
                    cat_stats.max_err_position = max(
                        cat_stats.max_err_position, abs(error[0])
                    )
                    cat_stats.max_err_orientation = max(
                        cat_stats.max_err_orientation, abs(error[1])
                    )
                else:
                    result_text = "misclassified"
                    cat_stats.misclassified[pr_object.category] = (
                        cat_stats.misclassified.get(pr_object.category, 0) + 1
                    )
                self._log(
    ...
    def _finalize_statistics(self, stats):
        ...
        train_progress_logger.info(
            f"{self._training_examples_done}\t{total_stats.get_text()}".replace(
                " ", ""
            ).replace("|", "\t")
        )
    ...
    def _object_diff(self, obj1, obj2):
        ...
        if (
            position_diff_pix > self._cfg["position_diff_thr"]
            or abs(orientation_diff) > self._cfg["angle_diff_thr"]
        ):
